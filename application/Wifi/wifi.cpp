...
ServerError Wifi::start_tcp_server(int port)
{
    ...
    */
    ...
    inet_aton("192.168.1.55", &server_socket_addr.sin_addr.s_addr); // inet_aton() converts the Internet host address cp from the IPv4 numbers-and-dots notation into binary form (in network byte order)
    server_socket_addr.sin_port = htons(12345);                     // The htons() function converts the unsigned short integer hostshort from host byte order to network byte order.

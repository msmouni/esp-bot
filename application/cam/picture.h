...
struct CamPicture
{
    ...
    // static const uint32_t MAX_FRAME_DATA_LEN = TcpIpServer::MAX_MSG_SIZE - SERVER_FRAME_HEADER_SIZE; // See Frame
    ...
    void setFrameHeader()
    {
        uint8_t header_buff[SERVER_FRAME_HEADER_SIZE] = {0};
        ServerFrame<SERVER_FRAME_HEADER_SIZE>::setHeader(header_buff, ServerFrameId::CamPic, MAX_FRAME_BUFFER_LEN, 0);

        memcpy(m_pic_frame_buff, header_buff, SERVER_FRAME_HEADER_SIZE);
    }
    CamPicture()
    {
        setFrameHeader();
    ...
    CamPicture(camera_fb_t *pic)
    {
        ...
        uint32_t len_to_copy = std::min((uint32_t)pic->len, (uint32_t)(MAX_FRAME_BUFFER_LEN - SERVER_FRAME_HEADER_SIZE));
        memcpy(&m_pic_frame_buff + SERVER_FRAME_HEADER_SIZE, pic->buf, len_to_copy);
    ...
    CamPicture(char *bytes)
    {
        ...
        memcpy(m_pic_frame_buff, bytes, (uint32_t)(MAX_FRAME_BUFFER_LEN - SERVER_FRAME_HEADER_SIZE));
